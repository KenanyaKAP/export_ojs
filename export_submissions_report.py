...
def fetch_authors(cur):
    """Return authors per submission: {submission_id: [(name, email), …]}."""
    cur.execute("""
        SELECT
            s.submission_id,
            CONCAT(
    ...
    for row in cur.fetchall():
        ...
        result[row["submission_id"]].append((name, email))
    return result
...
def fetch_section_editors(cur):
    ...
    Returns {submission_id: [(name, email), …]}.
    """
    cur.execute("""
        SELECT
            sa.submission_id,
...
def main():
    ...
    for i in range(1, max_authors + 1):
        header.append(f"Author {i} Name")
        header.append(f"Author {i} Email")
    for i in range(1, max_editors + 1):
        header.append(f"Section Editor {i} Name")
        header.append(f"Section Editor {i} Email")
    for i in range(1, max_reviewers + 1):
        header.append(f"Reviewer {i} Name")
        header.append(f"Reviewer {i} Email")
    ...
    with open(OUTPUT_CSV, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for row in submissions:
            ...
            # Authors (padded to max_authors)
            for name, email in authors_map.get(sid, []):
                csv_row.append(name)
                csv_row.append(email)
            csv_row.extend([""] * 2 * (max_authors - len(authors_map.get(sid, []))))

            # Section Editors (padded to max_editors)
            for name, email in editors_map.get(sid, []):
                csv_row.append(name)
                csv_row.append(email)
            csv_row.extend([""] * 2 * (max_editors - len(editors_map.get(sid, []))))

            # Reviewers (padded to max_reviewers)
            for name, email in reviewers_map.get(sid, []):
                csv_row.append(name)
                csv_row.append(email)
            csv_row.extend([""] * 2 * (max_reviewers - len(reviewers_map.get(sid, []))))

            writer.writerow(csv_row)

    print(f"✅ Exported {len(submissions)} submissions to {OUTPUT_CSV}")
    print(f"   Columns: 7 fixed + {max_authors}×2 authors"
          f" + {max_editors}×2 section editors"
          f" + {max_reviewers}×2 reviewers"
          f" = {len(header)} total")
