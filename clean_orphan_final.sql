...
-- NOTE: MySQL doesn't allow UPDATE on the same table used in a subquery,
--       so we wrap the subquery in a derived table to work around ERROR 1093.
UPDATE submission_files SET source_file_id = NULL
WHERE source_file_id IS NOT NULL
  AND source_file_id NOT IN (SELECT t.file_id FROM (SELECT file_id FROM submission_files) AS t);
...
-- ── 3.17 submission_tombstones → sections (NOT NULL — must DELETE) ──
-- preview: SELECT COUNT(*) FROM submission_tombstones WHERE section_id NOT IN (SELECT section_id FROM sections);
DELETE FROM submission_tombstones
WHERE section_id NOT IN (SELECT section_id FROM sections);

-- ── 3.18 library_files → submissions ──
-- NOTE: submission_id is NOT NULL in this schema, so we DELETE orphans instead of SET NULL.
--       submission_id = 0 means "not linked to a submission" in OJS, so we keep those.
-- preview: SELECT COUNT(*) FROM library_files WHERE submission_id != 0 AND submission_id NOT IN (SELECT submission_id FROM submissions);
...
-- ── 5.01 email_log → users (sender_id, NOT NULL — must DELETE) ──
-- preview: SELECT COUNT(*) FROM email_log WHERE sender_id NOT IN (SELECT user_id FROM users);
DELETE FROM email_log
WHERE sender_id NOT IN (SELECT user_id FROM users);

-- ── 5.01b email_log_users → email_log (re-clean after 5.01 DELETEs) ──
-- preview: SELECT COUNT(*) FROM email_log_users WHERE email_log_id NOT IN (SELECT log_id FROM email_log);
DELETE FROM email_log_users
WHERE email_log_id NOT IN (SELECT log_id FROM email_log);

-- ── 5.02 event_log → users (NOT NULL — must DELETE) ──
-- preview: SELECT COUNT(*) FROM event_log WHERE user_id NOT IN (SELECT user_id FROM users);
DELETE FROM event_log
WHERE user_id NOT IN (SELECT user_id FROM users);

-- ── 5.02b event_log_settings → event_log (re-clean after 5.02 DELETEs) ──
-- preview: SELECT COUNT(*) FROM event_log_settings WHERE log_id NOT IN (SELECT log_id FROM event_log);
DELETE FROM event_log_settings
WHERE log_id NOT IN (SELECT log_id FROM event_log);
...
-- ── 5.05b notification_settings → notifications (re-clean after 5.04/5.05 DELETEs) ──
-- preview: SELECT COUNT(*) FROM notification_settings WHERE notification_id NOT IN (SELECT notification_id FROM notifications);
DELETE FROM notification_settings
WHERE notification_id NOT IN (SELECT notification_id FROM notifications);
...
-- ── 6.15 filters → journals (context_id, NOT NULL — must DELETE) ──
-- preview: SELECT COUNT(*) FROM filters WHERE context_id NOT IN (SELECT journal_id FROM journals) AND context_id != 0;
DELETE FROM filters
WHERE context_id NOT IN (SELECT journal_id FROM journals)
  AND context_id != 0;
...
-- ── 6.17 filters → filters (parent_filter_id, NOT NULL, self-ref — must DELETE) ──
-- preview: SELECT COUNT(*) FROM filters WHERE parent_filter_id != 0 AND parent_filter_id NOT IN (SELECT filter_id FROM filters);
-- NOTE: MySQL doesn't allow DELETE on the same table used in a subquery,
--       so we wrap the subquery in a derived table to work around ERROR 1093.
DELETE FROM filters
WHERE parent_filter_id != 0
  AND parent_filter_id NOT IN (SELECT t.filter_id FROM (SELECT filter_id FROM filters) AS t);

-- ── 6.17b filter_settings → filters (re-clean after 6.15/6.16/6.17 DELETEs) ──
-- preview: SELECT COUNT(*) FROM filter_settings WHERE filter_id NOT IN (SELECT filter_id FROM filters);
DELETE FROM filter_settings
WHERE filter_id NOT IN (SELECT filter_id FROM filters);
...
-- ── 6.27 issue_galleys → issue_files (file_id, NOT NULL — must DELETE) ──
-- preview: SELECT COUNT(*) FROM issue_galleys WHERE file_id NOT IN (SELECT file_id FROM issue_files);
DELETE FROM issue_galleys
WHERE file_id NOT IN (SELECT file_id FROM issue_files);

-- ── 6.27b issue_galley_settings → issue_galleys (re-clean after 6.26/6.27 DELETEs) ──
-- preview: SELECT COUNT(*) FROM issue_galley_settings WHERE galley_id NOT IN (SELECT galley_id FROM issue_galleys);
DELETE FROM issue_galley_settings
WHERE galley_id NOT IN (SELECT galley_id FROM issue_galleys);
